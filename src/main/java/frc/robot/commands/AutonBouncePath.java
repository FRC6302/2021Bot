...
public class AutonBouncePath extends SequentialCommandGroup {
  ...
  static double voltageK = BatteryVoltage.getInitBatteryVoltage() / 12.8; //12.8 is max voltage I've seen
  ...
  public AutonBouncePath(DriveTrain driveTrain) {
    ...
    new Move(driveTrain, 0.22, 0.4, 1.91 * voltageK), //turns towards the first cone
    new Move(driveTrain, 0.3, 0.3, 0.21 * voltageK), //hits first cone
    new Move(driveTrain, 0, 0, 0.145 * voltageK), //makes robot slip less
    new Move(driveTrain, -0.4, -0.265, 2.46 * voltageK), //backs up towards next turn
    new Move(driveTrain, -0.4, -0.13, 1.97 * voltageK), //turns around cone
    new Move(driveTrain, -0.4, -0.26, 0.13 * voltageK), //makes robot jerk less
    new Move(driveTrain, -0.4, -0.4, 1.14 * voltageK), //hits second cone
    new Move(driveTrain, 0, 0, 0.3 * voltageK), //makes robot slip less
    new Move(driveTrain, 0.32, 0.4, 1.3 * voltageK) //0.75 0.8 moves towards next turn
